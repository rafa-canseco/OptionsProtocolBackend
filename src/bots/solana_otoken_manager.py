...
the MM can discover them via GET /mm/market.
...
# Anchor discriminator for create_otoken (from IDL)
...
def _get_program_ids() -> tuple[Pubkey, Pubkey]:
    """Return (otoken_factory_program, controller_program) pubkeys."""
    return (
...
def _find_or_create_otoken(
    ...
    """Find existing oToken or create new one. Returns mint address."""
    ...
    logger.info("Solana oToken created: %s tx=%s mint=%s", label, sig, otoken_mint)
    return str(otoken_mint)
...
def ensure_solana_otokens_exist(
    ...
    factory_program, controller_program = _get_program_ids()
    factory_config = _derive_factory_config(factory_program)
    controller_authority = _derive_controller_config(controller_program)
    ...
    for spec in specs:
        ...
        label = f"{asset.value} strike={spec.strike} expiry={spec.expiry_ts} {'put' if is_put else 'call'}"
