...
async def _wait_for_rpc(read_fn, check_fn, label: str) -> None:
    ...
    for attempt in range(RPC_SYNC_ATTEMPTS):
        ...
        if check_fn(result):
            return
        logger.debug(
            f"RPC sync waiting ({label}): attempt {attempt + 1}/{RPC_SYNC_ATTEMPTS}"
        )
...
class SettleResponse(BaseModel):
    settled: bool = Field(
        description="Whether the vault was successfully settled", examples=[True]
    )
    is_itm: bool = Field(
        description="Whether the option expired in-the-money", examples=[True]
    )
    settlement_type: Literal["physical", "cash", "physical_failed"] = Field(
        description="How the position was settled", examples=["physical"]
    )
    expiry_price: str = Field(
        description="Oracle ETH price at expiry (8 decimals, as string)",
        examples=["265000000000"],
    )
    settle_tx_hash: str = Field(
        description="Transaction hash of the batchSettleVaults call",
        examples=["0xabc123..."],
    )
    delivered_asset: str | None = Field(
        default=None,
        description="Address of asset delivered to user (physical settlement only)",
        examples=["0x4200000000000000000000000000000000000006"],
    )
    delivered_amount: str | None = Field(
        default=None,
        description="Amount delivered in native decimals (as string)",
        examples=["1000000000000000000"],
    )
    delivery_tx_hash: str | None = Field(
        default=None,
        description="Transaction hash of the physical delivery",
        examples=["0xdef456..."],
    )
    warning: str | None = Field(
        default=None,
        description="Warning message if settlement partially failed (e.g. physical delivery failed)",
    )
...
@router.post(
    ...
    summary="Trigger instant settlement (beta)",
)
async def demo_settle(
    body: SettleRequest,
    x_demo_key: str | None = Header(
        None, description="API key for demo settlement (required)"
    ),
):
...
async def _do_settle(body: SettleRequest) -> SettleResponse:
    ...
    if short_amount == 0:
        raise HTTPException(
            400, "Vault has no short position (already settled or empty)"
        )
    ...
    if body.force_itm is not None:
        ...
        if body.force_itm:
            ...
            oracle_price_8dec = (
                strike_price * 9 // 10 if is_put else strike_price * 11 // 10
            )
        else:
            # Force OTM: set price 10% above strike for puts, 10% below for calls
            oracle_price_8dec = (
                strike_price * 11 // 10 if is_put else strike_price * 9 // 10
            )
        logger.info(
        ...
        try:
            raw_answer, decimals, _updated_at = await asyncio.to_thread(
                get_eth_price_raw
            )
        except Exception:
    ...
    if needs_set:
        try:
            ...
            if "PriceAlreadySet" in str(e):
                logger.info(
                    f"Expiry price already set for {expiry}, reading on-chain value"
                )
            else:
    ...
    if is_itm:
        ...
        try:
            ...
            if allowance < short_amount:
                ...
                await _wait_for_rpc(
                    otoken_erc20.functions.allowance(
            ...
            delivered_asset = (
                settings.weth_address.lower()
                if is_put
                else settings.usdc_address.lower()
            )
