...
def _mock_web3(receipt_gas_used=42):
    ...
    w3.eth.send_raw_transaction.return_value = SimpleNamespace(
        hex=lambda: "0xdeadbeef"
    )
    receipt = MagicMock()
    receipt.status = 1
...
def test_sign_send_and_confirm_logs_label_and_gas_used(caplog):
    ...
    assert any(
        "Test tx label" in record.getMessage() and "gas_used=12345" in record.getMessage()
...
def test_sign_send_and_confirm_logs_even_when_gas_used_missing(caplog):
    ...
    assert any(
        "quirky chain tx" in record.getMessage() for record in caplog.records
    )
...
def test_build_and_send_tx_threads_label_to_sign_send():
    ...
    with patch("src.contracts.web3_client.get_w3"), patch(
