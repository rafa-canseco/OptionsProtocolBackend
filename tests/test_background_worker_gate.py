...
def _configure_all_workers(monkeypatch, *, enabled: bool) -> None:
    ...
    monkeypatch.setattr(settings, "base_otoken_manager_enabled", True)
    monkeypatch.setattr(settings, "base_event_indexer_enabled", True)
    monkeypatch.setattr(settings, "base_expiry_settler_enabled", True)
    monkeypatch.setattr(settings, "base_circuit_breaker_enabled", True)
    monkeypatch.setattr(settings, "yield_indexer_enabled", True)
    monkeypatch.setattr(settings, "bridge_relayer_enabled", True)
    monkeypatch.setattr(settings, "notification_bot_enabled", True)
    monkeypatch.setattr(settings, "solana_bots_enabled", True)
    monkeypatch.setattr(settings, "solana_circuit_breaker_bot_enabled", True)
    monkeypatch.setattr(settings, "solana_event_indexer_enabled", True)
    monkeypatch.setattr(settings, "solana_expiry_settler_enabled", True)
    monkeypatch.setattr(settings, "solana_otoken_manager_enabled", True)
...
def test_otoken_manager_can_start_without_other_base_workers(monkeypatch) -> None:
    _configure_all_workers(monkeypatch, enabled=True)
    monkeypatch.setattr(main_module.settings, "base_event_indexer_enabled", False)
    monkeypatch.setattr(main_module.settings, "base_expiry_settler_enabled", False)
    monkeypatch.setattr(main_module.settings, "base_circuit_breaker_enabled", False)
    monkeypatch.setattr(main_module.settings, "yield_indexer_enabled", False)
    monkeypatch.setattr(main_module.settings, "bridge_relayer_enabled", False)
    monkeypatch.setattr(main_module.settings, "notification_bot_enabled", False)
    monkeypatch.setattr(main_module.settings, "tokenized_fund_indexer_enabled", False)
    monkeypatch.setattr(main_module.settings, "rpc_snapshot_collector_enabled", False)
    monkeypatch.setattr(main_module.settings, "fund_nav_reporter_enabled", False)
    monkeypatch.setattr(main_module.settings, "legacy_agora_v1_enabled", False)
    monkeypatch.setattr(
        main_module,
        "has_bridge_config",
        lambda: (_ for _ in ()).throw(AssertionError("disabled bridge was validated")),
    )
    started_modules = []
    ...
    assert _run_lifespan_and_health_check() == {"status": "ok"}
    assert started_modules == ["src.bots.otoken_manager"]


def test_background_worker_settings_default_fail_closed(monkeypatch) -> None:
    for name in (
        "BACKGROUND_WORKERS_ENABLED",
        "BASE_OTOKEN_MANAGER_ENABLED",
        "BASE_EVENT_INDEXER_ENABLED",
        "BASE_EXPIRY_SETTLER_ENABLED",
        "BASE_CIRCUIT_BREAKER_ENABLED",
        "YIELD_INDEXER_ENABLED",
        "BRIDGE_RELAYER_ENABLED",
        "NOTIFICATION_BOT_ENABLED",
    ):
    ...
    assert settings.base_otoken_manager_enabled is False
    assert settings.base_event_indexer_enabled is False
    assert settings.base_expiry_settler_enabled is False
    assert settings.base_circuit_breaker_enabled is False
    assert settings.yield_indexer_enabled is False
    assert settings.bridge_relayer_enabled is False
    assert settings.notification_bot_enabled is False
