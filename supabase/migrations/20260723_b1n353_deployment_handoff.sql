ALTER TABLE v2_fund_registry
    DROP CONSTRAINT IF EXISTS v2_fund_registry_fund_key_key;

CREATE UNIQUE INDEX IF NOT EXISTS v2_fund_registry_enabled_fund_key_idx
    ON v2_fund_registry (fund_key)
    WHERE enabled;
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Target fund is already registered; use versioned bindings for upgrades';
    END IF;
...
GRANT EXECUTE ON FUNCTION v2_replace_fund_deployment(JSONB, JSONB) TO service_role;
